...
# Usage: python3 bismark_to_bigwig_pe_v3.py [-keep] [-sort] [-L=labels] [-p=num_proc] <chrm_sizes>  <allC_file> [allC_file]*
...
def processInputs( allCFileAr, chrmFileStr, keepTmp, labelsAr, outID, numProc, isSort ):
	print( 'Keep temp files: {:s}; Sort bedGraph: {:s}'.format( str( keepTmp), str (isSort)))
...
def printHelp():
	print ("Usage: python3 bismark_to_bigwig_pe_v3.py [-keep] [-sort] [-L=labels] [-o=out_id] [-p=num_proc] <chrm_sizes>  <bismark_file> [bismark_file]*")
